...
- Resources: Resource Group, Key Vault (RBAC auth), Cosmos DB (account + database `dev-news-db` + containers `news-items`, `short-videos`, `text-posts`), Function App (Flex Consumption `FC1`, `dotnet-isolated`, scale-out capped by `function_maximum_instance_count`), Static Web App, Application Insights + Log Analytics (conditional on `enable_application_insights`), a service-health Portal Dashboard (`dashboards.tf` + `dashboards/devnews-overview.json.tpl`, same `enable_application_insights` guard), Storage (containers `deployments`, `videos`, `thumbnails`), and a resource-group monthly budget alert (created only when `budget_alert_emails` is non-empty).
